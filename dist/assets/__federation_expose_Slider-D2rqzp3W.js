...
import { u as useResizeDetector } from './useResizeDetector-IEk-h4PO.js';
...
function Card({ index, onSizeChange, ...rest }) {
...
const variants = {
  ...
  center: ({ activeIndex, inactiveBlur, currentIndex, direction, x, inactiveScaleMultiplier }) => {
    let scale = 1;
    let opacity = 1;
    let blurValue = "0px";
...
function getZIndex({ activeIndex, currentIndex, direction }) {
  if (direction < 0) {
    return void 0;
  }
  if (currentIndex === activeIndex) {
...
const component = {
  ...
  render({ puck, options, slides = [] }) {
    ...
    return /* @__PURE__ */ jsxRuntimeExports.jsx(ContextSlider, { options, slides: slidesWithId, ref: puck.dragRef });
  }
};
